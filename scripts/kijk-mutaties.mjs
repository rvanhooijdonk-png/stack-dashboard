...
const MUTATIES = [
  ...
  },
  {
    ...
    van: "  { reden: 'INSPRINGING', test: (r) => inspringkolommen(r) >= 4 },",
    naar: '  { reden: \'INSPRINGING\', test: () => false }, // mutatie: inspringing is toegestaan',
  },
  {
    proef: '29-kolommen (reviewgat)',
    raakt: 'reviewgat 29 —',
    bestand: 'scripts/lib/kanaalpost.mjs',
    wat: 'de poort telt weer TEKENS in plaats van kolommen — " \\t| rij |" passeert en de scanner leest hem als echte toestand (bevinding Codex, zevende ronde)',
    van: "    else if (teken === '\\t') kolom += 4 - (kolom % 4);",
    naar: "    else if (teken === '\\t') kolom += 1; // mutatie: een tab is één kolom",
  },
  {
    ...
    van: "  if (/^[ \\t]*$/.test(r)) return 0; // alleen witruimte: voor markdown een lege regel",
