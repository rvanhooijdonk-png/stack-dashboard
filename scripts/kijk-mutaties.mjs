...
const MUTATIES = [
  ...
  },
  {
    ...
    van: "  { reden: 'INSPRINGING', test: springtIn },",
    naar: '  { reden: \'INSPRINGING\', test: () => false }, // mutatie: inspringing is toegestaan',
  },
  {
    proef: '29-tabinspringing (reviewgat)',
    raakt: 'reviewgat 29 —',
    bestand: 'scripts/lib/kanaalpost.mjs',
    wat: 'alleen een spatie telt nog als inspringing — een regel die met een TAB begint passeert en de scanner leest hem als echte toestand (bevinding Codex, zevende ronde, nu op de vereenvoudigde toets)',
    van: '  return /^[ \\t]/.test(r);',
    naar: '  return /^ /.test(r); // mutatie: een tab springt niet in',
  },
  {
    proef: '29-elkeinspringing (reviewgat)',
    raakt: 'reviewgat 29 —',
    bestand: 'scripts/lib/kanaalpost.mjs',
    wat: 'de grens ligt weer op vier kolommen — dan mogen kop, scheiding en rij op verschillende inspringing staan en valt de tabel voor markdown uiteen terwijl de scanner de rij leest (bevinding Codex, achtste ronde; 102 van 448 gemeten gevallen)',
    van: '  if (/^[ \\t]*$/.test(r)) return false; // alleen witruimte: voor markdown een lege regel\n  return /^[ \\t]/.test(r);',
    naar: "  if (/^[ \\t]*$/.test(r)) return false;\n  return /^(?: {4}|\\t)/.test(r); // mutatie: pas vier kolommen telt als inspringing",
  },
  {
    proef: '29-kaalmarkering (reviewgat)',
    raakt: 'reviewgat 29 —',
    bestand: 'scripts/lib/kanaalpost.mjs',
    wat: 'de containertoets eist weer een spatie ná de markering — een kale "-" op een eigen regel opent dan ongemerkt een lijstitem (bevinding Codex, achtste ronde)',
    van: "  { reden: 'CONTAINER', test: (r) => /^(?:>|(?:[-*+]|\\d{1,9}[.)])(?:[ \\t]|$))/.test(r) },",
    naar: "  { reden: 'CONTAINER', test: (r) => /^(?:>|(?:[-*+]|\\d{1,9}[.)])[ \\t])/.test(r) }, // mutatie: kale markering mag",
  },
  {
    proef: '29-witruimte (reviewgat)',
    raakt: 'reviewgat 29 —',
    bestand: 'scripts/lib/kanaalpost.mjs',
    wat: 'een regel met alleen witruimte telt weer als inspringing — vier spaties op een verder lege regel maken het hele logboek onleesbaar',
    van: "  if (/^[ \\t]*$/.test(r)) return false; // alleen witruimte: voor markdown een lege regel",
    naar: '  // mutatie: witruimte-regels tellen mee als inspringing',
  },
  {
    proef: '29-formfeed (reviewgat)',
    raakt: 'reviewgat 29 —',
    bestand: 'scripts/lib/kanaalpost.mjs',
    wat: 'de form feed valt uit de tekenklasse — hij zat er wel in maar werd nooit apart gemeten (bevinding Codex, zevende ronde)',
    van: "  { reden: 'REGELSCHEIDER', test: (r) => /[\\u2028\\u2029\\f\\v]/.test(r) },",
    naar: "  { reden: 'REGELSCHEIDER', test: (r) => /[\\u2028\\u2029\\v]/.test(r) }, // mutatie: form feed mag",
  },
  {
    proef: '29-verticaletab (reviewgat)',
    raakt: 'reviewgat 29 —',
    bestand: 'scripts/lib/kanaalpost.mjs',
    wat: 'de verticale tab valt uit de tekenklasse — zelfde verhaal als de form feed',
    van: "  { reden: 'REGELSCHEIDER', test: (r) => /[\\u2028\\u2029\\f\\v]/.test(r) },",
    naar: "  { reden: 'REGELSCHEIDER', test: (r) => /[\\u2028\\u2029\\f]/.test(r) }, // mutatie: verticale tab mag",
  },
  {
  ...
  },
  {
    ...
    raakt: 'reviewgat 30 —',
    bestand: 'scripts/lib/spiegelwet.mjs',
    wat: 'de schrijfpoort hakt weer op \\n alleen, dus een omgezet regeleinde maakt élke bestaande regel "nieuw" en een historische vuile regel blokkeert een keurige aanvulling (bevinding Gemini, achtste ronde)',
    van: "  const inRegels = (t) => String(t ?? '').split(/\\r\\n|\\r|\\n/);",
    naar: "  const inRegels = (t) => String(t ?? '').split('\\n'); // mutatie: CRLF blijft plakken",
  },
  {
    proef: '28a (reviewgat)',
    raakt: 'reviewgat 28 —',
