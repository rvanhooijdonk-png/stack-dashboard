...
/**
 * Lengte van de langste geladen deny-term. Nodig voor wie deze gate in stukken over een lange
 * tekst draait: elk ander patroon hier is begrensd, maar een deny-term is vrije mensentekst en
 * kan dus breder zijn dan de overlap tussen twee vensters (review Codex).
 */
export function denyTermsMaxLen() {
  // Reduce in plaats van spread: een lijst van duizenden termen zou als argumentenlijst
  // de stack overlopen (review Gemini, tweede ronde).
