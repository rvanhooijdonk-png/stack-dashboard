...
test('met BEIDE adressen expliciet draait de waarnemer buiten Actions, zonder identiteit', async (t) => {
  ...
  // Het statusbestand staat er als derde bij en dat is het bewijs erbovenop: het wordt afgeleid uit
  // het MEEGEGEVEN BASE_URL, dus het volgt deze plaatselijke server mee en vraagt niets over de
  // identiteit van deze repository. Stond het adres op github.io afgeleid, dan zou deze server het
  // nooit zien.
  assert.deepEqual([...bron.gezien].sort(),
    ['/contentstroom.html', '/kanaalpost-publiek.md', '/status.json']);
